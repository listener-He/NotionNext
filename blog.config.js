...
const BLOG = {
  ...
  THEME: process.env.NEXT_PUBLIC_THEME || 'hexo', // 当前主题；本 fork 仅保留 hexo 主题
  LANG: process.env.NEXT_PUBLIC_LANG || 'zh-CN', // e.g 'zh-CN','en-US'  see /lib/lang.js for more.
  SINCE: process.env.NEXT_PUBLIC_SINCE || 2021, // e.g if leave this empty, current year will be used.

  PSEUDO_STATIC: process.env.NEXT_PUBLIC_PSEUDO_STATIC || false, // 伪静态路径，开启后所有文章URL都以 .html 结尾。
  NEXT_REVALIDATE_SECOND: process.env.NEXT_PUBLIC_REVALIDATE_SECOND || 3600, // 更新缓存间隔 单位(秒)；即每个页面有纯静态期、此期间无论多少次访问都不会抓取notion数据；调大该值有助于节省Vercel资源、同时提升访问速率，但也会使文章更新有延迟。
  ...
  BLOG_FAVICON: process.env.NEXT_PUBLIC_FAVICON || 'https://cdn.jsdelivr.net/gh/listener-He/Home/favicon.ico', // blog favicon 配置, 默认使用 /public/favicon.ico，支持在线图片，如 https://img.imesong.com/favicon.png
  ...
  ...require('./conf/china-optimization.config'), // 中国大陆访问优化配置
  ...
  // 添加懒加载配置
  LAZY_LOAD_CONTENT: process.env.NEXT_PUBLIC_LAZY_LOAD_CONTENT || false, // 是否在构建时懒加载文章内容，可显著减少构建大小
}

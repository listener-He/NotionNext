...
export default async function getPageProperties(
  ...
  ).getTime() // 使用 .getTime() 确保是时间戳而不是 Date 对象
  properties.publishDay = formatDate(properties.publishDate, BLOG.LANG)
  properties.lastEditedDate = new Date(value?.last_edited_time).getTime() // 使用 .getTime() 确保是时间戳而不是 Date 对象
  properties.lastEditedDay = formatDate(
    new Date(value?.last_edited_time).getTime(), // 使用时间戳格式
    BLOG.LANG
  )

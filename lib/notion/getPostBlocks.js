...
import { CACHE_KEY_PAGE_CONTENT, CACHE_KEY_PAGE_BLOCK } from '@/lib/cache/cache_keys'
...
export async function getPage(id, from = null, slice) {
  ...
  // 缓存key包含slice参数，避免不同slice值之间的缓存冲突
  const cacheKey = CACHE_KEY_PAGE_CONTENT(id, slice)
  return await getOrSetDataWithCache(
    ...
    async (id, slice) => {
      ...
      if (pageBlock) {
        setDataToCache(cacheKey, pageBlock).catch(e => {
          // 静默处理缓存写入失败，不影响主流程
...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  ...
  if (retryAttempts && retryAttempts > 0) {
    ...
    try {
      ...
      await delay(1000)
      const cacheKey = CACHE_KEY_PAGE_BLOCK(id)
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  console.log('[API-->>批量请求]', `fetchInBatches query ids:${ids}`)
  // 如果 ids 不是数组或为空数组，则直接返回空对象

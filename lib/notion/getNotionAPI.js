...
function getRawNotion() {
  if (!globalStore.notion) {
    // 根据是否启用中国大陆优化选择不同的 API 地址
    const apiBaseUrl = process.env.NEXT_PUBLIC_CHINA_OPTIMIZATION_ENABLED === 'true' && process.env.NEXT_PUBLIC_NOTION_API_PROXY
      ? process.env.NEXT_PUBLIC_NOTION_API_PROXY

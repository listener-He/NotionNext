...
let FileCache, RedisCache, RedisCompressedCache, EmptyCache;
if (typeof window === 'undefined') {
  // 仅在服务端导入文件和Redis模块
  EmptyCache = require('./empty_cache').default;
...
export function getApi() {
  ...
  if (BLOG.REDIS_URL) {
    ...
    if (BLOG.ENABLE_REDIS_COMPRESSION) {
      return RedisCompressedCache || MemoryCache
    } else {
      return RedisCache || MemoryCache
    }
  } else if (process.env.ENABLE_FILE_CACHE) {
    // 服务端且启用文件缓存
    return FileCache || MemoryCache
  }

...
export async function setDataToCache(key, data, customCacheTime) {
  ...
  // 运行时关闭缓存时不写入（构建期仍需写入以生成静态产物缓存）
  if (!isBuildPhase && !cacheReadsEnabled(false)) return
...
/**
 * 清空所有缓存层（memory / redis / file）。
 * 用于 on-demand 全站刷新。运行时缓存链默认只有 memory（生产无 Redis 时），
 * 但这里对所有已知层都尝试清空，覆盖 build 产物与 Redis。
 */
export async function clearAllCache() {
  // 运行时链（生产：memory[+redis]）
  const stores = new Map(getCacheChain().map(c => [c.name, c.api]))
  // 始终确保 file 层也被清（build 期写入的文件缓存）
  stores.set('file', FileCache)

  for (const [name, api] of stores) {
    try {
      if (typeof api.clearAll === 'function') {
        await api.clearAll()
      }
    } catch (e) {
      console.warn(`[Cache] ${name} clearAll failed`, e.message)
    }
  }
  console.log('[Cache] clearAllCache done:', [...stores.keys()].join(', '))
}

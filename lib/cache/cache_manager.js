...
import { recordCacheOperation } from '../performance-monitor'
...
export async function getDataFromCache(key, force) {
  ...
  if (cacheEnabled || force) {
    ...
    if (!dataFromCache || JSON.stringify(dataFromCache) === '[]') {
      // 记录缓存操作
      recordCacheOperation('miss', key, 0)
      return null
    }
    // console.log(`[缓存命中]: ${key}`) // 减少日志输出
    // 记录缓存操作
    recordCacheOperation('hit', key, JSON.stringify(dataFromCache).length)
    return dataFromCache
  } else {
    return null
  }
...
export async function setDataToCache(key, data, customCacheTime) {
  ...
  try {
    ...
    // 记录缓存操作
    recordCacheOperation('set', key, JSON.stringify(data).length)
  } catch (error) {
    // 静默处理缓存写入失败
  }
}

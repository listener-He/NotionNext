      ...
      if (metadataLength > 0 && metadataLength < compressedData.length - 4) {
        ...
        if (metadata.isChunked && metadata.chunkCount && metadata.chunkSizes) {
          ...
          for (let i = 0; i < metadata.chunkCount; i++) {
            ...
            if (offset + chunkSize > compressedData.length) {
              throw new Error('Invalid chunk data: offset exceeds data length')
            }
          ...
          if (totalLength !== metadata.originalSize) {
            console.warn('⚠️ Decompressed size mismatch, but continuing...')
          }

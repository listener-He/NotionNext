...
function compressData(data, level = 3) {
  try {
    ...
    if (data.length > MAX_TOTAL_SIZE) {
      console.warn(
        `⚠️ Data too large for compression: ${(data.length / 1024 / 1024).toFixed(1)}MB > ${MAX_TOTAL_SIZE / 1024 / 1024}MB limit`
      )
    ...
    if (
      ...
      metadata.chunkSizes.length !== chunks.length
    ) {
      throw new Error('Metadata inconsistency: chunk count mismatch')
    }

    const totalCompressedSize = metadata.chunkSizes.reduce(
      (sum, size) => sum + size,
      0
    )
    const actualCompressedSize = chunks.reduce(
    ...
    const compressedMetadata = simple.compress(
    ...
    try {
      ...
      if (metadataLength > 0 && metadataLength < compressedData.length - 4) {
        ...
        if (
          ...
          Array.isArray(metadata.chunkSizes) &&
          metadata.chunkSizes.length === metadata.chunkCount
        ) {
          // 分块解压缩
          const chunks = []
          ...
          for (let i = 0; i < metadata.chunkCount; i++) {
            ...
            if (chunkSize <= 0 || offset + chunkSize > compressedData.length) {
              throw new Error(
                `Invalid chunk ${i}: size=${chunkSize}, offset=${offset}, total=${compressedData.length}`
              )
            }

            const compressedChunk = compressedData.slice(
              offset,
              offset + chunkSize
            )
            if (compressedChunk.length !== chunkSize) {
              throw new Error(
                `Chunk ${i} size mismatch: expected ${chunkSize}, got ${compressedChunk.length}`
              )
            }
            ...
            if (!decompressedChunk || decompressedChunk.length === 0) {
              throw new Error(
                `Failed to decompress chunk ${i} or chunk is empty`
              )
            }
          ...
          if (totalLength !== metadata.originalSize) {
            throw new Error(
              `Decompressed size mismatch: expected ${metadata.originalSize}, got ${totalLength}`
            )
          }
...
const CACHE_TIME_MULTIPLIERS = {
  PAGE_CONTENT: 1.5,     // 页面内容缓存系数
  PAGE_BLOCK: 1.2,       // 页面块缓存系数
  SITE_DATA: 2.0,        // 站点数据缓存系数
  AI_SUMMARY: 2.5,       // AI摘要缓存系数
  RSS: 1.0,              // RSS缓存系数
  DEFAULT: 1.5           // 默认缓存系数
}
...
function calculateCacheTime(key) {
  ...
  if (cacheInfo) {
    switch (cacheInfo.type) {
      case 'PAGE_CONTENT':
        multiplier = CACHE_TIME_MULTIPLIERS.PAGE_CONTENT
        break
      case 'PAGE_BLOCK':
        multiplier = CACHE_TIME_MULTIPLIERS.PAGE_BLOCK
        break
      case 'SITE_DATA':
        multiplier = CACHE_TIME_MULTIPLIERS.SITE_DATA
        break
      case 'AI_SUMMARY':
        multiplier = CACHE_TIME_MULTIPLIERS.AI_SUMMARY
        break
      case 'RSS':
        multiplier = CACHE_TIME_MULTIPLIERS.RSS
        break
      default:
        multiplier = CACHE_TIME_MULTIPLIERS.DEFAULT
    }
  }
...
export async function getCache(key) {
  ...
  try {
    ...
    // console.log(`[缓存命中]: ${key}, 数据类型: ${value.startsWith('ZSTD:') ? '压缩' : '未压缩'}, 大小: ${value.length} bytes`)
...
export async function setCache(key, data, customCacheTime) {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`Redis缓存写入: ${key} (TTL: ${cacheTimeToUse}s)`)
      if (compressionStats) {
        console.log(`  压缩统计: 原始${compressionStats.originalSize}B → 压缩${compressionStats.compressedSize}B, 比率${(compressionStats.compressionRatio*100).toFixed(1)}%, 时间${compressionStats.compressionTime}ms`)
      }
    }
...
export async function getCacheStats(keyPattern = '*') {
  ...
  try {
    ...
    return {
      ...
      compressionRatio:
        compressedKeys > 0
          ? compressedKeys / (compressedKeys + uncompressedKeys)
          : 0,
      connectionPoolStats: {
        activeConnections,
        queueLength: connectionQueue.length,

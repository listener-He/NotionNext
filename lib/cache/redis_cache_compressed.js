...
class RedisConnectionPool {
  constructor(config) {
    ...
    this.pool = {
      maxConnections: 48, // 降低最大连接数以避免资源浪费
      minConnections: 2,
      acquireTimeout: 10000,
      active: 4,
      waiting: [],
      connections: new Map(), // 追踪活跃连接和获取时间
      maxIdleTime: 600000, // 10分钟无活动则关闭连接
  ...
  async acquire() {
    return new Promise((resolve, reject) => {
      ...
      if (this.pool.active < this.pool.maxConnections) {
        ...
        const wrappedClient = this.createWrappedClient(connectionId);
  ...
  createWrappedClient(connectionId) {
    // 创建一个代理对象，将所有Redis方法调用转发到实际的Redis客户端
    const wrappedClient = new Proxy({}, {
      get: (target, prop) => {
        if (prop === 'release') {
          return () => this.release(connectionId);
        }
        // 如果是Redis客户端的方法，转发到实际客户端
        if (typeof this.client[prop] === 'function') {
          return (...args) => this.client[prop](...args);
        }
        // 否则直接返回客户端的属性
  ...
  release(connectionId) {
    ...
    if (this.pool.waiting.length > 0) {
      ...
      const wrappedClient = this.createWrappedClient(nextConnectionId);
      resolve(wrappedClient);
    }
  }

 ...
 */
export async function generateSitemap(input = []) {
  if (!['build', 'start'].includes(process.env.npm_lifecycle_event)) {
    console.log('[Sitemap] 跳过生成，非构建或启动阶段')
    return null;
  }
  const allPosts = Array.isArray(input)
    ? input
    : input?.allPages || input?.latestPosts || []

  const LINK = Array.isArray(input)
    ? siteConfig('LINK')
    : siteConfig('LINK', input?.siteInfo?.link, input?.NOTION_CONFIG)
  ...
  try {
    ...
    if (sitemapExists && timeDiff < sitemapCache.intervalMinutes) {
    ...
    console.error('[Sitemap] 读取文件失败:', error)
  }

  const allUrls = generateSiteAllUrl(LINK, currentDate, allPosts)
...
export function generateSiteAllUrl(LINK, currentDate, allPosts) {
  // 静态页面
  const staticPages = [
    {
      url: LINK,
  ...
  const postPages = allPosts
    ...
    .map(post => ({
      url: `${LINK}/${post.slug}`,
      lastmod: normalizeIsoDate(post, currentDate),
      changefreq: 'weekly',
      priority: '0.9'
    }))
  ...
  const pages = allPosts
    .filter(post => post.status === 'Published' && post.type === 'Page' && post.slug && !post.slug.startsWith('http://') && !post.slug.startsWith('https://'))
    .map(post => ({
      url: `${LINK}/${post.slug}`,
      lastmod: normalizeIsoDate(post, currentDate),
      changefreq: 'monthly',
  ...
  return [...staticPages, ...postPages, ...pages, ...categories, ...tags]
}
...
/**
 * 将 post 的多个可能的日期字段规范为 ISO 字符串
 * 优先顺序：lastEditedDate → publishDate → date.start_date → date.end_date
 * 无法解析则回退 currentDate
 */
function normalizeIsoDate(post, currentDate) {
  const candidates = [
    post?.lastEditedDate,
    post?.date,
    post?.publishDate
  ...
  for (const value of candidates) {
    if (typeof value === 'number' || typeof value === 'string') {
      try {
        ...
        // 忽略日期转换错误
        console.warn('日期转换错误:', value, error.message)
      }
    }
  }
  return currentDate

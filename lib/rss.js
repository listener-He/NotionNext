...
export async function generateRssFeed(props) {
  ...
  for (const post of limitedPosts) {
    ...
    // 限制内容长度
    if (content.length > RSS_MAX_CONTENT_LENGTH) {
      content = content.substring(0, RSS_MAX_CONTENT_LENGTH) + '...'
    }
    
    feed.addItem({
      title: post.title,
      link: `${LINK}/${post.slug}`,
      description: post.summary || content.substring(0, 200),
      content: content,

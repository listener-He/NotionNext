...
const generateAlgoliaSearch = async ({ allPages, force = false }) => {
  if (!allPages) {
    console.warn('Algolia: No pages provided')
    return
  }
  for (const p of allPages) {
    // 判断这篇文章是否需要重新创建索引
    if (p && (p.status && p.status === 'Published' && !p.password || force)) {
      await uploadDataToAlgolia(p)
    }
  }
}
...
const uploadDataToAlgolia = async post => {
  ...
  // 如果需要更新搜索
  if (needUpdateIndex) {
    // 减少内容大小限制，从8192降到5000字节，为其他字段留出空间
    const record = {
      objectID: post.id,
      title: post.title,
      ...
      content: truncate(getPageContentText(post, post.blockMap), 5000) // 减少索引字符数以避免超过限制
    }

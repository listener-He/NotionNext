...
/**
 * 加载外部资源
 * @param path 地址 例如 https://xx.com/xx.js
 * @param type js 或 css
 * @returns {Promise<unknown>}
 */
export function loadExternalResource(path, type = 'js') {
  const url = getCDNResourceSync(path);
  // 检查是否已存在
  const elements =
    type === 'js'
...
export function deepClone(obj) {
  if (obj && typeof obj === 'object') {
    try {
      if (obj.honestyCustomizeSourceCacheType &&  (obj.honestyCustomizeSourceCacheType === 'redis' || obj.honestyCustomizeSourceCacheType === 'redis-compressed')) {
        delete obj.honestyCustomizeSourceCacheType
        return obj
      }
    } catch (e) {
      console.warn('通过缓存来源类型判断是否需要深Copy异常', e)
    }
  }
  ...
  if (typeof structuredClone !== 'undefined') {
    try {
      ...
      // 如果 structuredClone 失败，继续使用下面的手动实现
      console.warn('structuredClone failed, falling back to manual implementation', e)
    }
  }

...
function getChinaMirror(resourceUrl, resourceType) {
  ...
  // jsDelivr 镜像
  if (resourceUrl.includes('cdn.jsdelivr.net')) {
    return resourceUrl.replace('https://cdn.jsdelivr.net', 'https://gcore.jsdelivr.net')
  }

  // // unpkg 镜像
  // if (resourceUrl.includes('unpkg.com')) {
  //   const unpkgMirror = siteConfig('UNPKG_MIRROR', 'https://unpkg.zhimg.com')
  //   return resourceUrl.replace('https://unpkg.com', unpkgMirror)
  // }

  // cdnjs 镜像
  if (resourceUrl.includes('cdnjs.cloudflare.com')) {
    const cdnjsMirror = siteConfig('CDNJS_MIRROR', 'https://cdn.bootcdn.net/ajax/libs')
    if (resourceUrl.includes('/ajax/libs/')) {
      // 处理标准的cdnjs库路径
      const match = resourceUrl.match(/.*\/ajax\/libs\/([^\/]+)\/([^\/]+)\/(.*)/)
      if (match) {
        return `${cdnjsMirror}/${match[1]}/${match[2]}/${match[3]}`
      }
      return resourceUrl.replace('https://cdnjs.cloudflare.com', cdnjsMirror)
    }
    return resourceUrl.replace('https://cdnjs.cloudflare.com', cdnjsMirror)
  }

  // Font Awesome 镜像
  if (resourceUrl.includes('kit.fontawesome.com') || (resourceUrl.includes('fontawesome') && resourceUrl.includes('css'))) {
    const fontAwesomeMirror = siteConfig('FONT_AWESOME_MIRROR', 'https://cdn.bootcdn.net/ajax/libs/font-awesome/6.4.0/css/all.min.css')
    if (resourceUrl.includes('kit.fontawesome.com')) {
      // 如果是 kit.fontawesome.com 的链接，替换为bootcdn的font-awesome
      return fontAwesomeMirror
    }
  }

  // 其他通用替换
  if (resourceUrl.includes('raw.githubusercontent.com')) {
    // 使用国内镜像
    return resourceUrl
      .replace('https://raw.githubusercontent.com', 'https://raw.githubusercontent.com.cn')

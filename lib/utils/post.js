...
import { getPageTableOfContents, getPageContentText } from '@/lib/notion/getPageTableOfContents'
import { getPostBlocks } from '@/lib/notion/getPostBlocks'
import { getRecommendPost } from '@/lib/notion/getRecommendPost'
import { countWords } from '@/lib/plugins/wordCount'
import { getAiSummary } from '@/lib/plugins/aiSummary'
import { uploadDataToAlgolia } from '@/lib/plugins/algolia'
import { getPageContentWordCount } from '@/lib/notion/getPageContentWordCount'
...
export async function processPostData(props, from) {
  ...
  if (allPosts && allPosts.length > 0) {
    ...
    // 获取上一篇和下一篇文章，只保留必要字段
     ...
     const prevPost = allPosts.slice(index - 1, index)[0] ?? allPosts.slice(-1)[0]
     const nextPost = allPosts.slice(index + 1, index + 2)[0] ?? allPosts[0]
    ...
    // 获取推荐文章，限制数量并只保留必要字段
     const maxRecommendPosts = getPerformanceConfig('DATA_OPTIMIZATION', 'MAX_RECOMMEND_POSTS', 4)

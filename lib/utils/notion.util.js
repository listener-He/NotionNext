...
export function adapterNotionBlockMap(blockMap) {
  ...
  for (const [id, block] of Object.entries(blockMap?.block || {})) {
    // Handle spaceId wrapping
    if (block && typeof block === 'object' && !block.value && !block.id) {
       for (const [innerId, innerBlock] of Object.entries(block)) {
          cleanedBlocks[innerId] = { value: unwrapValue(innerBlock) };
       }
    } else {
  ...
  for (const [id, collection] of Object.entries(blockMap?.collection || {})) {
     if (collection && typeof collection === 'object' && !collection.value && !collection.id) {
       for (const [innerId, innerCollection] of Object.entries(collection)) {
          cleanedCollection[innerId] = { value: unwrapValue(innerCollection) };
       }
    } else {

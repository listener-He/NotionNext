...
/**
 * 全站/全局数据（文章列表等）的缓存 TTL（秒）。
 * dev 下给一个短 TTL，避免文件缓存 expireTime=null 永不过期，导致新发/改的文章不出现在列表；
 * build/export 保持默认（构建期需跨步骤复用缓存）；生产运行时用 memory 10min TTL，不受此影响。
 */
const SITE_DATA_CACHE_TIME =
  process.env.npm_lifecycle_event === 'dev' ? 30 : null
...
export async function fetchGlobalAllData({
  ...
  from,
  locale,
  // 自定义：小程序 / RSS 等接口可指定只返回需要的数据类型以减小载荷
  dataTypes = null
}) {
  ...
  const cachedData = await getOrSetDataWithCustomCache(cacheKey, SITE_DATA_CACHE_TIME, async () => {
  ...
  // 自定义：按 dataTypes 裁剪返回体积（缓存后处理，避免污染共享缓存）
  return applyDataTypesFilter(data, dataTypes)
}

/**
 * 自定义：按调用方声明的 dataTypes 裁剪顶层重字段，减小 API 载荷。
 * block/schema/collection 等已在 handleDataBeforeReturn 中删除，此处仅裁剪
 * 体积较大的列表字段；未声明 dataTypes 时返回完整数据。
 * @param {object} data 已经过客户端清理的全站数据
 * @param {string[]|null} dataTypes 需要保留的数据类型
 */
function applyDataTypesFilter(data, dataTypes) {
  if (!Array.isArray(dataTypes) || dataTypes.length === 0) return data
  const optionalHeavyKeys = [
    'allPages',
    'allNavPages',
    'allLinkPages',
    'latestPosts',
  ...
  for (const key of optionalHeavyKeys) {
    if (!dataTypes.includes(key) && key in data) {
      delete data[key]
    }
  }
  return data
...
/**
 * 失效全站/站点级数据缓存（global_data_* 与 site_*）。
 * on-demand 刷新时调用，确保新增文章、标题/分类/标签改动能在下次访问时反映，
 * 而不必等待 memory 缓存 TTL 过期（修复全局数据缓存 key 无版本导致的滞后）。
 * @returns {Promise<string[]>} 已失效的 key 列表
 */
export async function clearGlobalDataCache() {
  const pageId = BLOG.NOTION_PAGE_ID
  const siteIds = pageId?.split(',') || []

  // 收集所有 locale：默认 + 配置语言 + 多站点前缀
  const locales = new Set(['default', BLOG.LANG])
  siteIds.forEach(siteId => {
    const prefix = extractLangPrefix(siteId)
    if (prefix) locales.add(prefix)
  })

  const keys = new Set()
  // global_data_{locale}_{pageId}（pageId 为完整逗号串，与 fetchGlobalAllData 默认一致）
  locales.forEach(locale => keys.add(getGlobalDataCacheKey({ pageId, locale })))
  // site_{id}（单站点级）
  siteIds.forEach(siteId => keys.add(`site_${extractLangId(siteId)}`))

  await Promise.all([...keys].map(k => delCacheData(k)))
  console.log('[revalidate] cleared global data cache keys:', [...keys].join(', '))
  return [...keys]
}
...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  const data = await getOrSetDataWithCustomCache(cacheKey, SITE_DATA_CACHE_TIME,
    async () => {
...
// 列表 summary 白名单：仅保留列表卡片/侧栏需要的字段。
// 瘦身：移除 pageCover（列表只用 pageCoverThumbnail，全尺寸 Notion 封面 URL 很长）与
// lastEditedDate（客户端列表不使用；文章正文缓存 TTL 在服务端按详情 post 计算）。
...
// 导航/链接页仅用于 URL 映射(convertInnerUrl 需 slug/href)、随机跳转(需 slug)等，
// 不渲染封面/摘要/标签。瘦身丢弃 summary(最大项)、cover、tags、category、日期，
// 大幅缩小每个页面都携带的 allNavPages/allLinkPages 体积。
const CLIENT_NAV_PAGE_FIELDS = [
  'id',
  'short_id',
  'title',
  'name',
...
function slimNavPage(page) {
  if (!page || typeof page !== 'object') return page
  const r = {}
  CLIENT_NAV_PAGE_FIELDS.forEach(f => {
    if (page[f] !== undefined) r[f] = page[f]
  })
  return r
}

function slimNavPages(pages) {
  if (!Array.isArray(pages)) return pages
  return pages.map(slimNavPage)
}
...
function handleDataBeforeReturn(db) {
  ...
  // 瘦身：allNavPages/allLinkPages 仅保留导航所需字段（丢弃 summary/cover/tags/日期）
  db.allNavPages = slimNavPages(cleanPages(db?.allNavPages, db.tagOptions))
  db.allLinkPages = slimNavPages(cleanPages(db?.allLinkPages, db.tagOptions))

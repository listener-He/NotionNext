...
import { idToUuid as originalIdToUuid } from 'notion-utils'

// 安全的idToUuid包装函数，确保输入是字符串类型
function idToUuid(id) {
  if (typeof id !== 'string') {
    // console.warn('idToUuid received non-string input:', typeof id, id)
    return id // 返回原值而不是undefined
  }
  try {
    return originalIdToUuid(id)
  } catch (error) {
    // console.error('idToUuid error:', error)
    return id
  }
}
...
export async function fetchGlobalAllData({
  ...
  from,
  locale, dataTypes = null // 新增参数，指定需要的数据类型
}) {
  // 确保 pageId 是字符串类型
...
export async function resolvePostProps({
  ...
  post = props?.allPages?.find(p => {
    ...
    return (
      ...
      (p.id && slugCandidates.has(idToUuid(p.id))) ||
      (typeof fullSlug === 'string' && slugCandidates.has(idToUuid(fullSlug)))
    )
  })
 ...
 */
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap, dataTypes = null) {
  if (!pageRecordMap) {
  ...
  // 根据数据类型决定是否处理分类数据
  let categoryOptions = []
  const allClass = getAllCategories({
    allPages,
    categoryOptions: getCategoryOptions(schema)
  })
  const categoryCount = Array.isArray(allClass) && allClass?.length || 0;
  if (!dataTypes || dataTypes.includes('categoryOptions')) {
    // 所有分类
    categoryOptions = allClass
  }
  ...
  // 根据数据类型决定是否处理最新文章
  let latestPosts = getLatestPosts({ allPages, from, latestPostCount: 6 })

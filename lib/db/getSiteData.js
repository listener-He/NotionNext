...
import { recordDataFetch, recordScheduledPublishCheck } from '../performance-monitor'
import { optimizedScheduledPublishCheck } from '../scheduled-publish-optimizer'
import { getPerformanceConfig, isDataOptimizationEnabled } from '@/lib/performance.config'
...
export async function getGlobalData({
  ...
  // 记录数据获取性能
  recordDataFetch('getGlobalData', 'full', duration, { 
    pageId, 
    from, 
...
function handleDataBeforeReturn(db) {
  ...
  if (POST_SCHEDULE_PUBLISH && Array.isArray(db.allPages)) {
    const result = optimizedScheduledPublishCheck(db.allPages, {
      enableLogging: true,
      batchSize: 100
    })
    ...
    // 如果检查被跳过（防重复），记录信息
    if (result.skipped) {
      console.log(`[定时发布] ${result.reason === 'duplicate_check_prevention' ? '跳过重复检查' : '检查被跳过'}`)
    }
  }
  ...
  return pages.map(page => {
    ...
    keepFields.forEach(field => {
      if (page[field] !== undefined) {
        optimizedPage[field] = page[field]
      }
    })

...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  return await getOrSetDataWithCache(
    ...
    async (pageId, from) => {
      ...
      const result = convertNotionToSiteData(pageId, from, deepClone(pageRecordMap))
      return result
    },
    pageId,
    from
...
function optimizePageData(pages) {
  ...
  // 导入性能配置
  const { getPerformanceConfig, isDataOptimizationEnabled } = require('@/lib/performance.config')
  ...
  // 获取需要保留的字段列表
  const keepFields = getPerformanceConfig('DATA_OPTIMIZATION', 'PAGE_LIST_FIELDS', [
    'id', 'title', 'slug', 'type', 'status', 'category', 'summary',

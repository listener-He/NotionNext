...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  // Notion旧图床
  if (needConvert) {
    // 确保 block.id 存在且为字符串类型
    const blockId = block?.id ? String(block.id) : 'unknown'
    ret =
      BLOG.NOTION_HOST +
      '/image/' +
  ...
  if (!isEmoji(ret) && ret.indexOf('notion.so/images/page-cover') < 0) {
    ...
    // 图片url优化，只对随机图片API添加唯一识别参数
    if (
      ret &&
      ret.length > 4 &&
      BLOG.RANDOM_IMAGE_URL &&
      BLOG.RANDOM_IMAGE_URL.trim().length > 0 &&
      ret === BLOG.RANDOM_IMAGE_URL
    ) {
      // 随机图片接口拼接唯一识别参数，防止请求的图片被缓存，而导致随机结果相同
      // 确保 block.id 存在且为字符串类型
      const blockId = block?.id ? String(block.id) : 'unknown'
      const separator = ret.includes('?') ? '&' : '?'
      ret = `${ret.trim()}${separator}t=${blockId}`
    }
  }
